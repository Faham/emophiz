...
void read_data(void)
{
  ...
  LONG liSamplesAvailable, liChannelHND;
    ...
    if( liSamplesAvailable ){
      variant = g_TTLLive->ReadChannelDataVT(liChannelHND, liSamplesAvailable );

			if(( variant.vt & VT_ARRAY ) == VT_ARRAY ){
				if(( variant.vt & VT_R4 ) == VT_R4){
					pSA = (variant.vt & VT_BYREF) ? *(variant.pparray) : variant.parray;
					if( ::SafeArrayLock(pSA) == S_OK){
							liSamplesAvailable = pSA->rgsabound[0].cElements;
							::SafeArrayUnlock(pSA);
					} else {
						liSamplesAvailable = 0;
					}
				}
			}
    }
		...
		if( liSamplesAvailable ){
			if (::SafeArrayLock(pSA) == S_OK) {
        FLOAT* data = (FLOAT*)pSA->pvData;
				::SafeArrayUnlock(pSA);
				printf("%c:%6.3f, ",(char)('A'+liChannelHND), data[0]);
			} else {
				printf("%c:ERR DATA, ",(char)('A'+liChannelHND));
			}
		} else {
